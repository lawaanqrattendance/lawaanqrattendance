        ...
        switch ($_POST['action']) {
            case 'add':
                ...
                try {
                    ...
                    // Insert into students table (with optional guardian_email)
                            ...
                            <td class="px-4 py-3 text-end">
                                <div class="btn-group btn-group-sm" role="group">
                                    <button type="button"
                                            ...
                                            data-guardian-email="<?php echo htmlspecialchars($student['guardian_email'] ?? ''); ?>"
        ...
        <div class="modal-content border-0 shadow-lg">
            ...
            <form id="addStudentForm" method="POST" class="needs-validation" novalidate>
                <div class="modal-body p-4">
                    ...
                    <div class="mb-4">
                        <label class="form-label fw-medium mb-2" for="studentId">Student ID <span class="text-danger">*</span></label>
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            <label class="form-label fw-medium mb-2" for="firstname">First Name <span class="text-danger">*</span></label>
                            <div class="input-group">
                                <span class="input-group-text bg-light border-end-0">
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content border-0 shadow-lg">
      <div class="modal-header bg-danger bg-opacity-10 border-0 py-3">
        <h5 class="modal-title text-danger fw-bold" id="deleteStudentModalLabel">
      ...
      <div class="modal-body p-4">
        <div class="alert alert-danger mb-3" role="alert">
          <strong>Warning:</strong> This will permanently delete the student, their user account, and all related records. This action cannot be undone.
        </div>
        <div class="mb-2">
          <strong>Student:</strong> <span id="deleteStudentName" class="text-primary"></span><br>
          <strong>ID:</strong> <span id="deleteStudentId" class="text-muted"></span>
        </div>
        <p class="mb-0">Are you sure you want to delete this student?</p>
      </div>
      <div class="modal-footer bg-light border-top-0 pt-0 px-4 pb-4">
        <button type="button" class="btn btn-outline-secondary rounded-pill px-4" data-bs-dismiss="modal">Cancel</button>
        <button type="button" class="btn btn-danger rounded-pill px-4" id="confirmDeleteStudent">
...
$(document).ready(function() {
    ...
    $('.edit-student').click(function() {
        ...
        const guardianEmail = button.data('guardian-email') || '';
        ...
        $('#edit_guardian_email').val(guardianEmail);
    ...
    // Delete Student with Bootstrap Modal
let studentToDeleteBtn = null;
let studentToDeleteId = null;

// When delete button is clicked, open modal and store info
$('.delete-student').click(function() {
    studentToDeleteBtn = $(this);
    studentToDeleteId = studentToDeleteBtn.data('id');
    const studentName = studentToDeleteBtn.closest('tr').find('td').eq(1).text().trim() || '';
    $('#deleteStudentName').text(studentName);
    $('#deleteStudentId').text(studentToDeleteId);
...
$('#confirmDeleteStudent').click(function() {
    ...
    const btn = studentToDeleteBtn;
    btn.prop('disabled', true);
    $('#confirmDeleteStudent').prop('disabled', true).html('<span class="spinner-border spinner-border-sm me-1" role="status" aria-hidden="true"></span> Deleting...');

    $.ajax({
        url: BASE_URL + '/admin/api/delete_student.php',
        method: 'POST',
        data: { student_id: studentToDeleteId },
        dataType: 'json',
        success: function(response) {
            $('#confirmDeleteStudent').prop('disabled', false).html('Yes, Delete');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let idAvailable = false; // always false until check passes

    if (!studentIdInput || !indicator || !message || !submitButton || !feedback) {
        console.warn('Required elements for student ID check are missing');
        // Always disable the button to prevent submission
        if (submitButton) submitButton.disabled = true;
        // Show a visible error
        if (feedback) {
            feedback.textContent = 'Student ID validation is not available. Please reload the page or contact support.';
        ...
        if (addForm) {
            addForm.addEventListener('submit', function(e) {
                e.preventDefault();
                e.stopPropagation();
                alert('Student ID validation is not available. Please reload the page.');
            });
        }
        return;
    }
    ...
    studentIdInput.addEventListener('input', function() {
        ...
        // While checking, always block submit
        idAvailable = false;
        submitButton.disabled = true;
        ...
        .then(data => {
            if (data && typeof data.available !== 'undefined') {
                if (data.available) {
                    ...
                    setInvalid('This Student ID is already taken. Please choose another.');
                }
            } else {
